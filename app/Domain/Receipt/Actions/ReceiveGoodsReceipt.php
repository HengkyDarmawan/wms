 ...
 *   saat ini barangnya masih milik gudang asal (BR-STK-13).
 *
...
class ReceiveGoodsReceipt
{
    ...
    public function handle(GoodsReceipt $receipt, ?User $actor = null): GoodsReceipt
    {
        ...
        $lines = $receipt->lines()->with('item.baseUom', 'shipmentLine')->orderBy('id')->get();
        ...
        return DB::transaction(function () use ($receipt, $lines, $actor) {
            $sj = $receipt->receipt_type === ReceiptType::Transfer
                ? Shipment::query()->withoutGlobalScopes()->with('warehouse')->findOrFail($receipt->shipment_id)
                : null;

            foreach ($lines as $line) {
                $receipt->receipt_type === ReceiptType::Transfer
                    ? $this->terimaTransfer($receipt, $sj, $line, $actor)
                    : $this->terimaVendor($receipt, $line, $actor);
            }
