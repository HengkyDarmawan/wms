...
| Dari | Ke | Aksi | Aktor | Guard | Efek |
|---|---|---|---|---|---|
| — | `prepared` (Disiapkan) | `shipment.create` | Staf Gudang | ≥1 PCK `completed` di Loading Area dengan tujuan sama (boleh dari beberapa REQ, [BR-SJ-09](05-aturan-bisnis.md#br-sj)); `shipment_method` terisi: `own_fleet` → kendaraan & driver, `carrier` → ekspedisi & resi, `self_delivered` → nama pembawa (transfer dalam proyek, [A-50](04-keputusan-dan-asumsi.md#a-50), [A-57](04-keputusan-dan-asumsi.md#a-57)). **Atau SJ tanpa PCK** ([A-247](04b-asumsi-lanjutan.md#a-247)): RET `approved` yang dijemput (tanpa stok Gudang Site), satu SJ per RET, `own_fleet`/`carrier` dengan sopir (user atau nama) dan plat (master atau bebas) wajib | — |
| `prepared` | `shipped` (Dikirim) | `shipment.ship` | Staf Gudang / Driver | Konfirmasi muat (foto opsional) | ledger: Loading Area → bin virtual *Dalam Perjalanan* (milik gudang asal); kejadian `goods_shipped`. SJ tanpa PCK: tanpa pergerakan & kejadian (barang di proyek) |
...
SJ jemput ([A-248](04b-asumsi-lanjutan.md#a-248)) tiba di gudang tujuan RET: `delivered`/`partially_delivered` tanpa pergerakan, tanpa DSC; yang tiba (baik + rusak) = batas GRN retur. Batal SJ jemput `prepared` → RET kembali `approved`.
...
| `approved` | `in_progress` | otomatis | sistem | PCK dibuat; **TRF aset** (`asset_onsite`): SJ antar site disusun ([A-249](04b-asumsi-lanjutan.md#a-249)) | — |
| `in_progress` | `completed` | otomatis | sistem | GRN tujuan `completed`; **TRF aset**: SJ antar site diterima proyek tujuan (aset yang tidak tiba tetap di proyek asal) | TRF aset: aset On-site asal → On-site tujuan, `asset_transferred` |
...
| `approved` | `in_progress` | otomatis | sistem | SJ balik dibuat — dari PCK stok Gudang Site atau SJ jemput tanpa PCK ([A-248](04b-asumsi-lanjutan.md#a-248)) — atau tanpa SJ bila diantar sendiri | — |
...
| `submitted`/`pending_approval`/`approved` (RET jemput juga `in_progress` selama SJ jemput `prepared`) | `cancelled` | `return.cancel` | Pengaju | Belum ada SJ `shipped`; alasan | SJ jemput `prepared` ikut `cancelled` |

Catatan implementasi ([22-retur-transfer](22-retur-transfer.md)): SJ balik stok Gudang Site lewat PCK ([A-111](04-keputusan-dan-asumsi.md#a-111)), barang lain lewat SJ jemput tanpa PCK, tidak dicampur dalam satu RET ([A-248](04b-asumsi-lanjutan.md#a-248)); GRN retur tanpa kejadian, kejadian terbit saat dipilah ([A-112](04-keputusan-dan-asumsi.md#a-112)); satu baris boleh dipilah ke beberapa hasil ([A-113](04-keputusan-dan-asumsi.md#a-113)); izin melihat `return.view` ditambah ([A-109](04-keputusan-dan-asumsi.md#a-109)).
...
| `checked_out` | `transferred` (Dipindahkan) | otomatis | sistem | SJ antar site TRF aset diterima proyek tujuan ([A-249](04b-asumsi-lanjutan.md#a-249)) | aset On-site asal → On-site tujuan; AST baru `checked_out` di proyek tujuan (`previous_handover_id`); kejadian `asset_transferred`; tanpa pemeriksaan |
| `returned` | `inspected` (Diperiksa) | `asset.inspect` | Staf Gudang | Grade kondisi + foto | state → `available` / `maintenance` / `damaged`; kejadian `asset_returned` terbit saat aset dipilah sesudahnya (lihat catatan) |
...
| `asset_handover_status` (AST, §2.11) | `checked_out` = Dipinjam · `returned` = Dikembalikan · `inspected` = Diperiksa · `transferred` = Dipindahkan (v0.21, [A-249](04b-asumsi-lanjutan.md#a-249)) |
...
| `stock_event_type` | Nilai = kolom kejadian di [matriks kejadian stok](05-aturan-bisnis.md#14-matriks-kejadian-stok): `goods_received` · `goods_rejected` · `goods_shipped` · `goods_delivered` · `goods_returned` · `stock_transferred` · `delivery_discrepancy` · `material_consumed` · `material_converted` · `waste_disposed` · `stock_adjusted` · `asset_checked_out` · `asset_returned` · `asset_transferred` (v0.21, [A-249](04b-asumsi-lanjutan.md#a-249)) · `asset_lost_or_damaged` · `purchase_requested` · `purchase_request_cancelled` |
